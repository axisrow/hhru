...
2. **Policy-гейт до клика** (ядро #929 переиспользуется, якоря не
   ...
   **#1220 (боевой census testing 2026-09-24)**: drop-панель пикера резюме
   ...
   понижает и ambiguous-предка — ТОЛЬКО когда на странице есть видимый
   apply_step-overlay (форма отклика открыта; context
   `apply_flow_picker_overlay`). `dangerous` не понижается никогда; без
   флага (этап 1, авто-режимы) поведение не изменилось: тот же ambiguous
   отказывает, защита #1215 цела.
