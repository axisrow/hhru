...
|---|---|---|
| `dangerous` | Текстовые якоря опасности: captcha/«не робот», удалить/отозвать, необратим, «вы уверены», подтвердить/confirm | Никогда. Только репорт |
| `apply_step` | Форма отклика (`RESPONSE_MODAL_FORM_ID`, `data-qa*="vacancy-response"` — оба shape), тест-вопросы (`task-question`/`task-body`), тексты «сопроводительное письмо/тестовое задание/анкета/отклик» | Никогда. Часть сценария отклика |
| `safe` | Toast/notification/cookie-баннер по природе; modal/overlay — только при явном close-контроле | Разрешён dismiss |
...
«подтвердите согласие» попадёт в `dangerous` — осознанный fail-closed.
...
`data-qa`/class `close`, глиф `×` у интерактивного элемента. «Понятно»,
...
- cookie-баннер hh.ru (его close-маркеры) — доступен анонимно, но живой
  осмотр в объём этого MVP не входил.
