...
2. **Policy-гейт до клика** (ядро #929 переиспользуется, якоря не
   ...
   **allowApply (#1162)**: явная команда сценария отклика понижает apply_step-
   отказ самой цели (context `apply_flow`) и её overlay-предка (context
   `apply_flow_overlay`) — это решение агента, как явное имя цели.
   **#1220 (боевой census testing 2026-09-24)**: drop-панель пикера резюме
   ...
   семейства модалки отклика (текст панели совпал с overlay боевого отказа
   дословно; якорей и close-контролов у панели нет — честный ambiguous), и
   «apply_step-предок в семействе» там отсутствует. Поэтому allowApply
   понижает и ambiguous-предка — ТОЛЬКО когда overlay — сама панель пикера
   (data-qa `drop-base` / класс `magritte-drop-base`) И на странице есть
   видимый apply_step-overlay, то есть форма отклика открыта (context
   `apply_flow_picker_overlay`). Посторонний ambiguous-диалог рядом с
   apply_step отказывает и с allowApply (ревью Codex PR #1221);
   `dangerous` не понижается никогда; без флага (этап 1, авто-режимы)
   поведение не изменилось: тот же ambiguous отказывает, защита #1215
   цела.
