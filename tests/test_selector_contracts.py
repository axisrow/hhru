...
def test_every_selector_has_canonical_coverage_and_all_groups_are_audited():
    ...
    # #826: +1 resume_page.RESUME_SKILLS_SUGGEST_USER_INPUT — the autocomplete
    # option that echoes the typed skill name; Enter never committed a chip in
    # this combobox (confirmed live on both an empty and a non-empty skills
    # section), only clicking this option does.
    # #821: the exact count lives in selectors/count.txt (regenerated by
    # `selector_contracts.py render`, same pipeline as _generated.py and
    # reference-matrix.md) instead of a literal here, so two parallel PRs
    # that both add a selector no longer collide on this line.
    assert len(catalog["selectors"]) == contracts.load_selector_count()
    assert all(
...
def test_bootstrap_output_passes_catalog_gate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(contracts, "COUNT_PATH", tmp_path / "selectors" / "count.txt")
    ...
    contracts.COUNT_PATH.write_text(contracts.render_count(catalog), encoding="utf-8")
...
def test_refresh_dry_run_reports_baseline_and_never_writes(monkeypatch, capsys, tmp_path):
    ...
    assert f"local selector contracts: {contracts.load_selector_count()}" in output
