...
class _OtherCardsLocator:
    ...
    bump после таймаута якоря карточки подтверждает видимость списка и
    повторно ищет карточку (гидрация-гейт, ревью PR #1079). На экране этого
    фейка список отрисован и стабилен (wait_for — безусловный успех),
    карточки недоступного резюме в нём нет — подтверждённое отсутствие,
    count()>0 достаточен для вердикта «не найдено в списке».
    """
    ...
    @property
    def first(self) -> _OtherCardsLocator:
        return self

    def wait_for(self, *, state: str = "visible", timeout: float = 0) -> None:  # noqa: ARG002
        """Список на этом экране уже отрисован — ожидание проходит сразу."""
