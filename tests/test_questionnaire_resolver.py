...
@pytest.mark.parametrize(
    ...
    [
        ...
        # «Ведение» зарплаты — тот же вопрос о чужом опыте, что и «расчёт»
        # (признак взят из конкурирующего PR #496, где он был выведен
        # независимо; проверен по корпусу — 0 регрессий).
        "Есть ли у вас опыт ведения заработной платы?",
        "Опыт ведения зарплаты сотрудников?",
        "Ведение заработной платы сотрудников?",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Тот же инвариант для «расч»/«начисл»: считать можно не только
        # зарплату. На базовом f686c5a эти вопросы отвечались корректно — без
        # привязки к зарплатному объекту контр-признак их регрессировал.
        "Есть ли у вас опыт расчета unit-экономики? Какие зарплатные ожидания?",
        "Опыт расчета нагрузки? Зарплатные ожидания?",
        "Есть ли опыт начисления бонусов партнёрам? Ваши зарплатные ожидания?",
    ],
)
def test_exclude_does_not_veto_expectations_about_other_experience(text):
    """Контр-признак без зарплатного объекта — не повод глушить шаблон (#490).

    В отличие от «расчёта» и «начисления», которые сами по себе про деньги,
    «ведение» бывает какое угодно: проектов, команды, документации. Признак,
    принимающий любой объект, вето́рует весь salary-seed, и настоящий вопрос об
    ожиданиях уходит в очередь — ровно та цена, которой контр-признаки должны
    избегать. Поэтому стем требует явного зарплатного объекта вплотную.
    """
    match = match_keyword(text)
...
def test_vedeni_stem_does_not_swallow_svedeniy():
    """Граница ``\\b`` у стема «ведени» обязательна (признак из PR #496).

    Без неё «ведени» ловится внутри «с\\ *ведений», и вопрос о достоверности
    сведений о зарплате — обычный зарплатный вопрос — молча уходил бы в
    очередь. Тест держит именно эту границу: она и есть причина, по которой
    стем можно было позаимствовать.
    """
    match = match_keyword("Достоверность сведений о зарплате?")
