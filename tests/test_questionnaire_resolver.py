...
@pytest.mark.parametrize(
    ...
    [
        ...
        # #490: тема (\bзарплат) не значит намерение спросить желаемую сумму —
        # это вопрос про ОПЫТ расчёта чужой (сотрудников) зарплаты.
        "Есть ли опыт расчета зарплаты сотрудников?",
        "Опыт начисления заработной платы сотрудникам?",
        "Ведение расчета зарплаты сотрудников — ваши обязанности?",
    ],
)
...
@pytest.mark.parametrize(
    "text",
    [
        # Корпусные salary-вопросы (#490 регресс): exclude не должен задеть их.
        "Ваши зарплатные ожидания?",
        "Укажите желаемую зарплату",
        "Зарплата?",
        "Какая зарплата вас интересует?",
        "Ваши пожелания по окладу",
        "Желаемый оклад",
        "Заработная плата?",
        "уровень доходов",
        "Уровень оплаты труда",
        "Сколько вы хотите зарабатывать?",
        # Регресс на границу слова у exclude-стема ``ведени`` (#490 ревью):
        # без \b перед альтернацией «ведени» ловит «сведений»/«заведениях» —
        # словоформы, не имеющие отношения к расчёту чужой зарплаты.
        "Подтверждаете ли достоверность сведений о зарплате сотрудников?",
        "Опыт работы в заведениях, зарплата, сотрудники",
    ],
)
def test_salary_exclusion_does_not_shadow_real_expectation_questions(text):
    """#490: exclusion для 'опыт расчёта чужой зарплаты' узкий намеренно — не
    должен задеть реальные вопросы про желаемую зарплату кандидата."""
    match = match_keyword(text)
