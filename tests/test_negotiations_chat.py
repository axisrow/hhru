...
#
# Живые факты 2026-09-08: <работодатель-1> «ГигаРекрутер» ответил в ту же
# минуту (12:29 → 12:29), <работодатель-2> — через 14 минут; человек-образный
# ответ <работодатель-3> — 2ч14м. Время пузыря — только HH:MM (суточное окно).
...
def test_reply_within_threshold_is_robot():
    assert is_robot_questionnaire(_pair("12:22", "12:36"))  # <работодатель-2>, 14 мин


def test_slow_reply_is_not_robot_by_speed():
    """2ч14м (<работодатель-3>, <рекрутер-1>) — скорость молчит; этого детекта хватает
