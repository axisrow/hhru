...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Трёхбуквенный корень: Snowball не имеет отдельного порога длины,
        # но должен стемить короткие слова так же надёжно, как длинные.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("технический", "Вакансия технического директора"),
        ("продуктовый", "Поиск продуктового аналитика"),
        ("нейронные", "Опыт нейронных сетей"),
        ("сети", "Опыт нейронных сетей"),
        ("технологии", "Знание современных технологий"),
        ("данные", "Работа с данными"),
    ],
)
def test_russian_adjective_and_plural_forms_match(skill, vacancy_text):
    """Snowball закрывает измеренный пробел на прилагательных и формах мн. числа."""
    outcome = resume_match_score(card(vacancy_text), profile(skills=[skill]))
    assert outcome.score_0_100 > 0.0, f"{skill!r} должен матчить {vacancy_text!r}"
