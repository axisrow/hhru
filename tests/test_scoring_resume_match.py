...
@pytest.mark.parametrize(
    ("skill", "vacancy_text"),
    [
        ("системный", "Требуется системного аналитика"),
        ("ведущий", "Ищем ведущего разработчика"),
        ("нейронные", "Опыт нейронных сетей"),
        ("технологии", "Знание современных технологий"),
    ],
)
def test_known_gap_adjective_forms_do_not_match_yet(skill, vacancy_text):
    """ЗАФИКСИРОВАННАЯ ГРАНИЦА, а не желаемое поведение (ревью #501, цикл 3).

    Самодельная таблица окончаний надёжна на существительных, но снимает
    неравные куски с падежных форм прилагательных: «системный» -> «систем»,
    «системного» -> «системн». На корпусе обычных ролей это 43% промахов.

    Тест закрепляет факт, чтобы он не потерялся: когда стеммер заменят на
    выверенный (Snowball/pymorphy), этот тест УПАДЁТ — и это будет правильным
    сигналом переписать его в положительный, а не признаком регрессии.
    """
    outcome = resume_match_score(card(vacancy_text), profile(skills=[skill]))
    assert outcome.score_0_100 == 0.0
