...
3. **Двухуровневый троттлинг** в `throttle.py`:
   ...
   - **Попап подтверждения переезда** (#1135, дамп `apply_136789544_form_timeout.html`
     2026-09-16 + 187 боевых срабатываний 09-13/09-16): триггер — из текста самого
     попапа: резюме НЕ помечено «готов к переезду» + вакансия в другой стране
     относительно резюме (заголовок «Вы откликаетесь на вакансию в другой стране»;
     про другой город той же страны данных нет). DOM: Magritte-alertdialog на самой
     странице вакансии (модалка, canonical `/vacancy/{id}` не меняется), generic-контейнер
     `data-qa='magritte-alert'`, якоря `relocation-warning-{title,confirm,abort}`;
     кнопки — настоящие `<button>`: confirm «Все равно откликнуться» (primary),
     abort «Отменить» (secondary, кодом не используется). **Гонка монтажа:** попап
     может монтироваться в окне 1,5–8 с после клика — позже обоих блокер-проходов;
     form-timeout путь в `navigate_to_response_form` делает повторный проход блокеров
     до вердикта form_timeout (иначе вакансия сгорает в `uncertain`, блокирующем
     повтор по `has_unresolved_uncertain`), а при включённом флаге после клика даёт
     форме второй бюджет ожидания. **Семантика клика:** `search.allow_relocation: true`
     кликает подтверждение — это МУТАЦИЯ профиля (hh.ru сам меняет сигнал на «готов к
     переезду», боевой факт 2026-09-19, принято владельцем в #1132 как осознанная
     цена). Флаг действует ТОЛЬКО в боевом пути: в dry-run гейтится
     (`ctx.allow_relocation and not ctx.dry_run`) — сухой прогон получает честный
     relocation-блокер со skip-строкой, которую считает stats-счётчик (#1149). Успех
     после клика несёт в `actions.reason` пометку «попап переезда подтверждён кликом»
     (`pipeline._relocation_confirmed_suffix`) — без неё срабатывания при включённом
     флаге невидимы. SSR-флага «покажется ли попап» не существует (дамп проверен) —
     предсказание до клика не реализуемо без чтения профиля.
   - **Модалка «Контакты в резюме могли устареть»** (#1189, read-only census
     /applicant/resumes 2026-09-20): аккаунтовый Magritte-alert на magritte-overlay,
     смонтирован и ВИДИМ при простом заходе на список резюме, до любых кликов —
     перехватывал клик поднятия по hit-target check (боевой сбой #1161: 30с таймаут
     клика → acted+uncertain и кулдаун 4ч впустую). Кнопки — настоящие `<button>`:
     primary accept `profile-contacts-sync-alert-accept` «Заменить на новые из
     профиля» — МУТАЦИЯ профиля (контактов), кодом не адресуется никогда;
     secondary cancel `profile-contacts-sync-alert-cancel` «Закрыть» — dismiss,
     не мутация (в теле ишью кнопка названа «Оставить», в живом DOM — «Закрыть»).
     Обработка — ветка (а): `blockers.close_stale_contacts_alert` закрывает
     dismiss-кнопкой и продолжает; в bump вызывается прямо перед кликом поднятия
     с коротким render-wait монтажа (боевой прогон 2026-09-20: чистый успех за
     8с, но клик на ~5-й секунде ОБГОНЯЛ монтаж попапа — в census-визитах на 6-й
     секунде он уже виден; без бюджета быстрый прогон кликает впритык к
     перехвату), dry-run гейтится выходом до боевого блока — ноль кликов, success
     несёт пометку в reason; в apply-семействе пост-клик блокеров попап даёт явный
     skip `stale_contacts_alert` (post_navigation наследуется — «серая зона» #207),
     не безликий form-timeout uncertain. #1188: сбойный клик поднятия
     (PlaywrightError) в bump тоже не безликий — после close-попытки
     `bump._post_click_verdict` читает состояние кулдауна на карточке
     (внешний источник истины, паттерн #207): hint отрисован → success;
     кнопка активна → определённый failed (hh.ru перехваченный клик не
     засчитывал, кулдаун 4ч не начинается); ни один сигнал не подтверждён →
     прежний fail-closed uncertain (#176). Бюджет ожидания сигналов —
     `BUMP_POST_CLICK_STATE_WAIT_MS` (5с, SPA-перерисовка после поднятия
     быстрая); ложный failed безопасен — повторный bump hh.ru отклонит
     «рано поднимать» сам. На странице вакансии модалки нет
     (census 2026-09-20, 0 вхождений); серверный эффект dismissal (станет ли
     попап показываться снова) read-only недоказуем — подтвердит боевой прогон.
     На той же странице есть НЕмодальный инлайн-баннер
     `profile-contacts-sync-banner` с той же accept-кнопкой — клики не
     перехватывает, кодом не адресуется.
...
6. **Попытка засчитывается ровно один раз, в ровно одном месте, по структурному
   ...
   - **Мутирующий клик визарда создания резюме — финальный NEXT после
     каталога, не первый** (#936, живые факты 2026-09-03; опровергает
     гипотезу #920 от 2026-09-02). Боевой прогон: title введён, первый
     NEXT выполнен с ПУСТЫМ каталогом (attempted=1) — read-only
     `list-resumes` сразу, через 1.5 и через 5 минут следа НЕ нашёл.
     Наблюдалось ранее (#920, ручной уход с экрана каталога: черновик с
     title и пустой ролью) — механикой визарда не воспроизводится;
     правдоподобное объяснение того случая — autosave при уходе из
     мастера, а не первый NEXT. Рабочий механизм проведения профессии
     ВНЕ каталога — тоже финальный NEXT: выбрать в дереве плейсхолдер
     «Другое» (id 40), экран `professional_role` закрывается ролью 40
     (`nextIncompleteScreenId=educations`), черновик материализуется
     (`--allow-unresolved-area`, приёмочный прогон 2026-09-03). Следствия:
     (а) `before_click`-seam `create_resume_on_hh` стоит ровно на
     финальном NEXT для обоих путей; (б) отказ по профессии после первого
     NEXT печатает фантом-подсказку консервативно, но «attempted=1» при
     пустом каталоге следа не оставляет; (в) подтверждение черновика —
     readback `list-resumes`/SSR (`not_finished`, роль 40), а не поиск
     «пустой роли».
...
### Наблюдение страницы — census, не grep по дампу (критично)

Агент «слеп»: HTML-дамп неотличим от UI, а JSON-состояние и i18n-словари
страницы содержат текстовые литералы будущих форм («Город», «График работы»
— #998, 8 ложных «Городов»). Вывод «поле есть/нет» — ТОЛЬКО по
отрисованным контролам:

- `census --url <URL>` — таблица отрисованных контролов
  (data-qa/tag/role/label/text/visible), read-only, «глаза» агента;
- любой дамп `dump_page_html` пишет рядом `.census.txt` и печатает census
  видимых контролов в лог — разбор отказа начинай с него, не с grep
  по `.html`: вхождение строки в дамп НЕ доказательство существования поля;
- семантику контрола определяет подпись на экране, а не имя атрибута
  (`work-ticket-selector` оказался «Разрешением на работу», #997);
- сырой HTML/outerHTML в stdout и логах ЗАПРЕЩЁН — только файл-дамп:
  скрытые узлы и литералы в атрибутах агент читает как поля; в stdout —
  census/таблица (аудит #1002, probe negotiations);
- `aria-expanded`/`aria-selected` НЕ доказательство состояния контрола на
  hh.ru: решение о клике — по факту (счёт `role=option`, появление leaf-строк,
  повторное чтение), не по атрибуту. Механически подтверждённая причина сбоев —
  гидрация: видимый SSR-узел до гидрации клики молча теряет (#858; #840 — тот
  же случай с month-комбобоксом). «Magritte не обновляет aria-атрибут» —
  гипотеза БЕЗ живого подтверждения: единственный дамп #840 фиксирует штатное
  `aria-expanded` false -> true (`selector_groups/resume_experience.py`);
  live-сверка aria-семантики — #1004.
...
- **Подтверждено боевым дампом + массовыми боевыми срабатываниями** (`vacancy_page.py`,
  #1135): `[data-qa="relocation-warning-confirm"]` — попап «Вы откликаетесь на вакансию
  в другой стране» (дамп `apply_136789544_form_timeout.html` 2026-09-16 + 187 боевых
  срабатываний 09-13/09-16; кнопки `relocation-warning-confirm` «Все равно откликнуться»
  и `relocation-warning-abort` «Отменить», обе — `<button>`). Детали и семантика —
  архитектурный п.3. Из списка «заимствовано, НЕ подтверждено» ниже исключён.
- **Заимствовано из reference-проектов, НЕ подтверждено живым DOM** (`vacancy_page.py`,
  #342/PR #435, waiver владельца репозитория): оставшиеся шесть пост-кликовых блокеров
  hh.ru (похожие вакансии, внешний отклик, `negotiations-limit-exceeded`,
...
**Визард черновика: автопубликация ровно на последнем незакрытом экране (#900,
прогноз #1012).** Порядок экранов стабилен: common → educations → keyskills →
(skill_levels — только у черновиков с навыками, живой факт 2026-09-07) →
experience; hh.ru публикует черновик сам при сабмите ПОСЛЕДНЕГО незакрытого
экрана — подтверждено двумя независимыми прогонами (#1009 «Дворник-бригадир»
и «Повар» 2026-09-06: educations/keyskills прошли без публикации, experience
опубликовал). Отсюда: `wizard-next` требует `--allow-auto-publish` только для
`experience` (`is_publishing_screen`); промежуточные NEXT публикуть не могут и
флага не требуют. **Таймаут навигации на публикующем экране не равен отказу
(#1038, дважды подряд 2026-09-07/08):** финальная публикация hh.ru может
завершиться редиректом, не наступающим как navigation до `commit` в 30-секундном
бюджете — локальный таймаут ничего не доказывает. `submit_wizard_screen` перед
вердиктом uncertain на публикующем экране делает identity-bound readback
(`read_resume_state` + `is_published`): публикация подтверждена — success,
иначе uncertain (fail-closed #176). URL-паттерн пост-публикационного редиректа
не используется — вместо догадки внешний источник истины, как в #207.
Правило дизайна гейтов: **гейт = предсказание с источником** —
если команда печатает «не уверен, произойдёт ли X», но X выводим из уже
прочитанного состояния, чинится код, а не добавляется флаг-разрешитель.
...
### Сессия и воркеры (агентские прогоны)

hh.ru-сессия живёт в **файле** `data/storage_state/hh_session.json` (куки), а не
в каком-либо браузере. CLI-команды читают его сами (`launch_context` загружает
storage_state в контекст) — отдельного «входа» перед прогоном не существует.

- Воркеру (LLM-агенту), увидевшему форму входа в открытом браузере: это ошибка
  адресации, а не повод для `login` — интерактивный login требует человека
  (пароль/2FA) и автоматизации не подлежит. Засейте куки из storage_state в
  свой профиль (`add_cookies`, #1194) или запускайте CLI с конфигом аккаунта —
  сессия подхватится из файла. Прецедент: воркер #1186 сделал 9+ попыток
  «залогиниться»; после засева кук тот же контур прошёл живой прогон.
- `hhtoken` есть в jar, но сервер отдал форму входа — storage_state устарел:
  пересев того же файла не поможет, нужен свежий `login` человеком или
  `import-cookies` из залогиненного Chrome.
- Гейт `browser.require_authenticated_page` (`has_auth_cookie() and not
  has_login_form()`, #147/#1129) корректен и честно отказывает; текст
  `NotAuthenticated` несёт оба рецепта (#1194).
...
- CI гоняет сюиту под `pytest-xdist` (`-n 4 --dist worksteal`) и проверяет её через
  ...
  CI сверяет `ruff check`, `ruff format --check` и актуальность
  `docs/cli-reference.md` (`scripts/gen_cli_docs.py --check`).
