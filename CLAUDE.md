...
7. **Анкеты отвечаются обучаемыми шаблонами, а LLM — только последняя ступень**
   ...
   - **Вся история анкет ключуется реальным `resume_id`** (hex-хвост
     ...
     **Статус в UNIQUE не входит**, поэтому перед переносом слаг-строка сливается
     ...
     вакансию.
     Маппинг проверяется ДО первой мутации и при неоднозначности **внутри
     текущего конфига** не выполняется вовсе: `AppConfig.get_resume` прямо поддерживает коллизию
     ...
     ренейм по такой паре унёс бы законные строки чужого резюме под первый
     ключ, смешав ответы анкет и аудит, а совпавшие `question_key` `DELETE`
     удалил бы навсегда. Отказ целиком, а не наполовину: провенанс
     перемешанных строк потом не восстановить. Пара «слаг == СВОЙ же
     `resume_id`» коллизией не является — иначе нормализованный конфиг
     отменял бы уборку и для остальных резюме. **Проверка видит только резюме
     текущего конфига, но не историю**, а та durable и переживает правки
     `config.yaml`: слаг, совпавший с каноническим ключом выбывшего из конфига
     резюме, не детектится. Ровно так же перенос **не сравнивает `updated_at`**
     и позволяет старой слаг-строке затереть более новую hex-строку. Обе дыры
     — варианты одной посылки «ключ строки доказывает её происхождение и
     свежесть», которая неверна; разбор и варианты решения — в #513, и до него
     `rekey_*` считать безопасным нельзя.
