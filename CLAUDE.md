...
- `aria-expanded`/`aria-selected` НЕ доказательство состояния контрола на
  hh.ru (magritte может не обновлять атрибут — #840): решение о клике — по
  факту (счёт `role=option`, появление leaf-строк, повторное чтение), не по
  атрибуту; видимость SSR-узла до гидрации тоже не «кликабельность» (#858).
