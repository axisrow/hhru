...
**Визард `professional_role` после `copy-resume` имеет отдельный live-контракт**
(подтверждено живым DOM 2026-08-31): копия сбрасывает `professionalRole` и открывает
пустой экран с `role: null`, `title: null`, без input профессии и без chip-ов. На
следующем chip-экране `resume-profile-position-input` не сохраняет введённый текст:
это фильтр-подсветка списка из 37 общих `resume-profile-position-chip-popular`, а не
поле произвольной профессии; совпавший общий chip переезжает первым и получает
`checked=true`. Точная профессия `role_id=96` через этот путь недостижима: она не
входит в список общих chip-ов и отсутствует в их узком подсписке. Поэтому роли вроде
«Грузчик» или SMM/CMO в таком прогоне — выбранные общие chip-категории, а не
унаследованные значения. Точную специализацию задавать только через editor
