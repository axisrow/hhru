...
6. **Попытка засчитывается ровно один раз, в ровно одном месте, по структурному
   ...
   - **Первый NEXT визарда создания резюме МАТЕРИАЛИЗУЕТ черновик на hh.ru**
     (#920, живой факт 2026-09-02). Ввод должности + клик «Продолжить» на
     первом экране — и hh.ru уже создаёт сущность резюме (в списке: «Не
     опубликовано», «Дополнить»), хотя профессия не выбрана и финальный NEXT
     не нажимался: title сохранён, роль пуста (SSR: `fields: {title: …}`,
     незакрытый экран `professional_role`). Следствие: отказ в каталоге
     после первого NEXT (в т.ч. все отказные ветки
     `select_wizard_catalog_leaf`) оставляет фантомный черновик с голым
     title — `[FAIL]` команды честен про профессию, но след на hh.ru есть.
     Классификация «точка невозврата = финальный NEXT» ниже по тексту
     описывает исходы CLI (uncertain/failed), а не момент появления
     сущности: после любого отказа из визарда проверять `list-resumes` и
     убирать фантом `delete-resume` (ручная дисциплина, автозатирания
     чужих черновиков нет).
