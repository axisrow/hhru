...
7. **Анкеты отвечаются обучаемыми шаблонами, а LLM — только последняя ступень**
   ...
   - **Вся история анкет ключуется реальным `resume_id`** (hex-хвост
     ...
     из `learn` (WRITE): маппинг «слаг → resume_id» живёт в конфиге, до которого
     история не достаёт. Тестовый двойник резюме обязан иметь РАЗНЫЕ `id` и
     `resume_id` — одинаковые прячут расхождение.
