...
7. **Анкеты отвечаются обучаемыми шаблонами, а LLM — только последняя ступень**
   ...
   - **Вся история анкет ключуется реальным `resume_id`** (hex-хвост
     ...
     **Статус в UNIQUE не входит**, поэтому перед переносом слаг-строка сливается
     ...
     повторяет `record_questionnaire_pending` (ON CONFLICT DO UPDATE) **целиком,
     а не только по статусу**: слаг-строку писал probe+learn уже с распознанным
     ...
     (`_learn_one` берёт `row["cluster"]` вторым приоритетом). Приоритет
     совпадает с `record_questionnaire_pending` (заново увиденный вопрос
     возвращает строку в работу и обновляет её поля), и обратным ему быть
     не должен.
