...
7. **Анкеты отвечаются обучаемыми шаблонами, а LLM — только последняя ступень**
   ...
   - **Вся история анкет ключуется реальным `resume_id`** (hex-хвост
     ...
     **Статус в UNIQUE не входит**, поэтому перед переносом активная слаг-строка
     «воскрешает» свой hex-близнец (`status='pending'`) — иначе `UPDATE OR IGNORE`
     отказался бы переносить её из-за `resolved`-близнеца, а `DELETE` уничтожил бы
     вопрос навсегда. Приоритет тот же, что у `record_questionnaire_pending`
     (заново увиденный вопрос возвращает строку в работу), и обратный ему быть
     не должен.
