...
Заполняет аттестации, рекомендации и сертификаты по подтвержденным UI-маршрутам; умеет создать первую строку в пустом блоке. Портфолио и ссылки пока пропускаются, удаления не выполняются. Ключ manual в секции resume_sections конфига пока зарезервирован и ни на что не влияет — расцветёт вместе с блоковыми ишью #1119-1122.
