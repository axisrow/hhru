...
Любое исключение после клика ловится широким `except Exception` (`:302`) и пишется как
`failed`. Слова `uncertain` в модуле **нет ни разу**.
...
**Последствие не косметическое.** `history.count_today()` считает только `success`/`uncertain`
(`history.py:424`), поэтому запись `failed` не расходует лимит и не дедуплицируется: повторный
