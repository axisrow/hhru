...
#
# Эмпирическая валидация #853 (2026-09-02, 75 прогонов за сутки): дефолтный
# порог 5с подтверждён — спокойный hh.ru ходит за 2.6-4.5с (p95 4.06с),
# baseline за 1.3-3.1с, ложных срабатываний порога нет (0/110 проб). Две
# границы применимости, найденные тем же прогоном:
# 1. DDoS-Guard может перевести curl в JS-челлендж (быстрый 403 с jsx-страницей
#    вместо 200 с контентом) — тогда time_total меряет edge DDoS-Guard, а не
...
# основание (#853, живой кейс 2026-09-02: google.com transport_fail десять
...
if [ "$baseline_all_fast" = true ] && [ "$hh_is_slow" = true ]; then
    ...
    if [ "$baseline_all_fast" = true ]; then
        ...
        echo "ВЕРДИКТ: inconclusive -> данные baseline противоречивы (часть быстрая, часть медленная/недостижима), уверенный диагноз невозможен; повторите прогон, когда маршрут восстановится."
    fi
fi
