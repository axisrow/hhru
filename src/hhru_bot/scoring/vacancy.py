"""Скоринг релевантности вакансии кандидату: эвристика #15 + LLM (#74).

**ScoringProvider / LLMScoringProvider** — «ML простым путём»: LLM (#16
...
``hermes-agent-axisrow`` не нужен для импорта этого модуля — LLMClient лениво
тянет его только в момент построения (см. ``hhru_bot.ai.letters``).
"""
