...
Чистая функция без браузера и без сети: взвешенное пересечение токенов
``AIProfile`` и ``VacancyCard.vacancy_text``. Для русской морфологии используется
лёгкий чисто-Python Snowball stemmer (Тир 1 по issue #508).
...
2. **Матч строгий токенный с ограниченным стеммингом, а не подстрочный.**
   ...
   словоформами. Решение: оба токена нормализуются выверенным русским Snowball
   stemmer и сравниваются на РАВЕНСТВО.
   ...
   стем-нормализации). Snowball определён только для русского алфавита и
   латинские токены оставляет без изменений, поэтому «java» ~ «javascript»
   не матчатся без всяких числовых порогов.
...
def _stem(token: str) -> str:
    """Сводит токен к стему проверенным русским алгоритмом Snowball.

    Латинские токены Snowball не изменяет, поэтому «java»/«javascript» и
    «react»/«reactive» остаются разными стемами.

    Экземпляр создаётся на вызов: API Snowball хранит текущее слово в объекте,
    поэтому локальный экземпляр сохраняет чистоту функции и потокобезопасность.
    Это дёшево для чисто-Python алгоритма без словарей и моделей.
    """
    return snowballstemmer.stemmer("russian").stemWord(token)


# Стем некоторых стоп-слов (Snowball) короче их словоформ: «опыт» -> «оп»,
# но «опыта» -> «опыт» — стем инфлексии совпадает с исходным словом, а не с
# его собственным (over-)стемом. Без самих стоп-слов в этом множестве «опыта»
# не распознавалась бы как стоп-слово, оставаясь токеном между маркером
# отрицания и навыком и разрывая узкое окно ``_is_negated`` (#490 регрессия,
# обнаружена после ужесточения окна в #550).
_STOPWORD_STEMS = frozenset(_STOPWORDS) | frozenset(_stem(word) for word in _STOPWORDS)
...
def _tokens_match(profile_token: str, vacancy_token: str) -> bool:
    ...
    Snowball остаётся алгоритмическим стеммером, а не морфологическим
    анализатором: редкие омонимии и совпадающие стемы возможны. Это приемлемый
    компромисс для лёгкой зависимости обычного пути; полноценный словарный
    морфоанализатор здесь не нужен.
    """
