...
2. **Матч строгий токенный с ограниченным стеммингом, а не подстрочный.**
   ...
   словоформами. Компромисс: стем-матч разрешён только от ``_MIN_STEM_LEN``
   символов И при расхождении длин не больше ``_MAX_ENDING_LEN`` — короткие
   токены («go», «1с») сравниваются строго, а слова с общим началом, но
   разным смыслом («java»/«javascript», «план»/«планета») не матчатся, хотя
   порог длины стема сам по себе их пропускал бы.
...
# Максимальная РАЗНИЦА длин при стем-матче. Именно она, а не длина стема,
# отделяет словоформу от чужого слова с общим началом:
#
#   словоформа   «разработчик» -> «разработчика»   +1
#                «тестирование» -> «тестировании»   0  (окончание замещается)
#                «инженер» -> «инженеры»            +1
#   чужое слово  «react» -> «reactive»              +3
#                «план» -> «планета»                +3
#                «java» -> «javascript»             +6
#
# Русское склонение окончание ЗАМЕЩАЕТ, а не дописывает, поэтому типичные пары
# укладываются в 0-1 символ, а несвязанные слова начинаются от +2 («банк» ->
# «банкет», «карта» -> «картина»).
#
# Разделение неидеально, и цена известна: формы вида «сайт» -> «сайтов» (+2)
# порог тоже отсекает, то есть часть настоящих совпадений недосчитывается.
# Выбор осознанный — на +2 живут и словоформы, и чужие слова, а направление
# ошибки при недосчёте безопаснее: тот же fail-closed принцип, что у отрицания
# (#490). Полноценное решение — сворачивание по списку окончаний вместо
# сравнения префиксов; на Этапе 1 (только наблюдение) это не окупается, но при
# калибровке Этапа 2 порог/подход стоит пересмотреть по реальному распределению.
_MAX_ENDING_LEN = 1
...
def _tokens_match(profile_token: str, vacancy_token: str) -> bool:
    ...
    Точное равенство — всегда. Сверх него разрешён ТОЛЬКО общий стем, и
    ограничений на него два, оба обязательны:

    1. **Оба токена не короче ``_MIN_STEM_LEN``** — иначе короткие токены («go»,
       «1с») ложно матчили бы длинные слова с теми же первыми буквами.
    2. **Расхождение не длиннее ``_MAX_ENDING_LEN`` символов.** Одной проверки
       (1) мало: длины порога хватает и несвязанным словам с общим началом
       («java» ~ «javascript», «план» ~ «планета», «админ» ~ «администратор»),
       и такой матч ЗАВЫШАЕТ score — направление, прямо противоречащее
       fail-closed принципу модуля, и загрязняющее распределение, ради
       наблюдения за которым сделан Этап 1. Стемминг введён под русскую
       морфологию («разработчик» ~ «разработчика»), а окончание — это несколько
       символов, а не целое слово: ограничение по длине хвоста разделяет эти
       два случая, тогда как порог длины стема — нет.

    Матч по-прежнему двусторонний: профиль может нести как более полную форму
    («тестирование» в профиле vs «тестировать» в вакансии), так и более
    короткую.
    """
    ...
    if len(profile_token) < _MIN_STEM_LEN or len(vacancy_token) < _MIN_STEM_LEN:
        return False
    shorter, longer = sorted((profile_token, vacancy_token), key=len)
    if not longer.startswith(shorter):
        return False
    return len(longer) - len(shorter) <= _MAX_ENDING_LEN
