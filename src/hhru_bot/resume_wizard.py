...
def _read_validation_rejection(page: Page, target: str) -> str | None:
    ...
    Граница допущения (ревью PR #1097): проверка не различает ошибки,
    отрендеренные ЭТИМ отклонённым кликом, и ошибки, которые hh.ru мог
    отдать SSR до клика (например, после неудачной попытки в другой
    сессии). Допущение держится, потому что _open_screen делает свежий
    goto перед каждым сабмитом, а пустая форма получила бы те же ошибки
    повторно; если кейс проявится — симптом «честный failed» при реально
    ушедшем клике, и тогда нужен маркер «ошибка свежая» (сравнение до/
    после клика), не расширение селектора."""
    try:
