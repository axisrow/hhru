...
def register(subparsers) -> None:
    ...
    parser.add_argument(
        "--dry-run", action="store_true", default=True, help="Только показать состав"
    )
    parser.add_argument("--apply", action="store_true", help="Выполнить восстановление")
...
def _restore(args: argparse.Namespace) -> None:
    names = restore_backup(args.archive, args.config, args.history, dry_run=not args.apply)
    for name in names:
        print(name)
    if not args.apply:
