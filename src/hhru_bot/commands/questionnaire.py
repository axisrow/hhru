...
def _answer_cell(row: dict) -> str:
    ...
    ГРАНИЦА ТОЧНОСТИ, и она же — причина разделения колонок. ``filled`` фиксирует
    успешное ЗАПОЛНЕНИЕ формы, но не подтверждает отправку. Отдельная колонка
    ``исход`` берётся из ``actions`` и показывает исход отклика для того же
    ``run_id + resume_id + vacancy_id``; она не меняет смысл ``filled``.
...
def _delivery_cell(row: dict) -> str:
    """Return the action outcome without conflating unknown and no action."""
    return {
        "success": "success",
        "uncertain": "uncertain",
        "failed": "failed",
        "unknown": "[исход неизвестен]",
        "no_action": "[действия нет]",
    }.get(row["delivery_status"], "[исход неизвестен]")
...
def run_audit(args: argparse.Namespace) -> None:
    ...
    print(
        _ascii_table(
            ["вакансия", "conf", "источник", "шаблон", "вопрос", "ответ", "исход"],
            [
                [
                    row["vacancy_id"],
    ...
    # ложь, что и ячейка, этажом ниже. Исход отклика оставляем построчным:
    # одна анкета может содержать несколько вопросов, но action у неё один.
