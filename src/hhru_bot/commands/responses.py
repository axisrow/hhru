...
# Backstop адаптивного --max-pages (#1148). Радиус — ВСЯ команда responses,
# не только read-only просмотр (раскладка по режимам — ниже). Конец списка
...
# только дальше — включая strict-пути --sync-applied/--alert-new: их контракт
# полноты не ослаблен, дефолт просто 5 -> 50. paginated_*-пути
# (--remindable/--detect-external-tests) обрезаются молча, как и раньше, —
# но теперь до 50 GET вместо 5 (их дотейфовый silent-truncation контракт без
# стража #1074 — отдельный follow-up). Бюджет clear-negotiations (необратимая
# команда, инвариант «отказ до любого клика», PR #196) не затронут — у него
# свой argparse.
