...
def register(subparsers) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "поля формы; preferred=True максимум у одной строки и "
            "переставляет радио предпочтительного способа на этот контакт "
            "(снимает с другого); без preferred существующий выбор hh.ru "
            "не меняется (#1124). "
...
def run(args: argparse.Namespace) -> None:
    ...
    if supported:
        ...
        # Слив исходов боевого прохода в outcomes выполняет _sync_row_outcomes
        # внутри launch_context (см. выше) — дубль слива здесь убран
        # (cycle-review PR #1127 r5).
    failed = 0
    if outcomes is not None:
