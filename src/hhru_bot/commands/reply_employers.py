...
def _run(args: argparse.Namespace, config, history, progress: ApplyProgress) -> bool:
    ...
    with launch_context(
        ...
        for candidate in candidates:
            ...
            # раньше, чем locally-stale candidate до него дойдёт.
            #
            # #762 (ручная браузерная проверка, 2026-08-30): сам целевой
            # сценарий -- allowed=true у чата, где рекрутер написал последним
            # и мы молчим -- в проверенной выборке (95 SSR-топиков аккаунта,
            # 19 с allowed=true) НЕ встретился, поэтому вопрос "снимает ли
            # hh.ru allowed именно после РУЧНОГО ответа" остался неподтверждён
            # и неопровергнут. Зато обнаружен смежный факт: allowed=true
            # эмпирически держится ТОЛЬКО пока в чате нет вообще ни одного
            # сообщения от собеседника ("отклик без ответа") и падает в false,
            # как только приходит любое сообщение с той стороны -- живого
            # рекрутера или даже бота "ИИ-помощник hh" (проверено на 3 живых
            # чатах). needs_follow_up срабатывает только когда диалог уже БЫЛ
            # (chat.author == "me" после чьего-то входящего) -- то есть именно
            # в тех чатах, где allowed уже, по этим наблюдениям, false. Отсюда
            # риск, не подтверждённый на 100% (выборка не покрыла диалоги с
            # несколькими репликами), но и не противоречащий коду/тестам:
            # remindable_topics может систематически исключать все настоящие
            # кандидаты --follow-up, а не только защищать от дублирования
            # поверх ручного ответа. Логику сознательно не меняем без более
            # прямого подтверждения (см. issue #762) -- но если --follow-up
            # на боевом аккаунте стабильно ничего не находит ("hh.ru не
            # разрешает" на каждом чате), первый подозреваемый -- не баг,
            # а эта эмпирика: remindable_topics пуст не потому что напоминать
            # не о чем, а потому что allowed так и не бывает true в диалогах
            # с обменом репликами. Исходное предположение (снимает ли hh.ru
            # allowed именно после ручного ответа) по-прежнему не опровергнуто:
            # если обнаружится дублирующее напоминание поверх ручного ответа,
            # первый подозреваемый остаётся оно.
            #
            # #762 этап 2 (testing-аккаунт, 2026-09-08): живой кейс с
            # подтверждёнными ручными ответами -- chat 5610565556, три
            # сообщения от нас после входящих (последнее -- ответ на отказ),
            # viewedByOpponent=true. После них responseReminderState.allowed
            # == false, т.е. пина поверх живого диалога --follow-up на этом
            # кейсе НЕ даёт. Строго приписать снятие флага именно ручному
            # ответу по-прежнему нельзя: бот написал за ~2 часа до ответа,
            # а по эмпирике этапа 1 allowed падает уже после первого
            # входящего, так что флаг мог быть false и до. Третья живая
            # точка обратного случая: чат, где рекрутер-человек написал
            # последним и мы молчим (INTERVIEW) -- allowed тоже false.
            # Суммарно 6 живых точек (этап 1 + 2) в одну сторону: allowed
            # = true наблюдается только у безответных откликов.
            dedup_marker = (
