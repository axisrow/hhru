...
hhru-специфичный резолвер: один endpoint из ``config.yaml`` (секция ``ai``)
+ ключ из окружения. Кредитный роутинг hermes (пулы, OAuth, ~20 провайдеров)
не используется — контроль расходов остаётся на стороне hhru (#230).
...
def resolve_runtime_provider(
    ...
    *,
    ...
    Args:
        ...
        api_mode: optional override; defaults to ``responses``.
