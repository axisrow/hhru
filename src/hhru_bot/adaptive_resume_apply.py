...
в теле issue #769 как допустимое сужение первой версии). Причина — не в
качестве этих данных, а в состоянии двух соседних задач прямо сейчас:
1. ``read_experience_on_hh`` (единственная функция чтения, на которой можно
   было бы строить сопоставление company+position между ``candidate_facts``
   конфига и реальными строками hh.ru) переписывается воркером по #844 —
   строить сопоставление на функции, которую параллельно чинят от потери
   строк, значит целиться в движущуюся цель.
2. Разведка по #787 (в процессе) установила, что опыт работы на hh.ru живёт
   в общем профиле (``/applicant/profile/me``), а не привязан однозначно к
   конкретному ``resume_id`` — сама модель "записать опыт в резюме X" под
   вопросом. WRITE по опыту до ответа #787 рискует стать молчаливым no-op:
   команда вернула бы ``[OK]``, а на целевом резюме ничего не изменилось бы
   (прямое нарушение fail-closed принципа проекта).
Follow-up после #844/#787 может реализовать company+position matching в духе
уже отклонённого здесь плана (см. обсуждение issue #769).
