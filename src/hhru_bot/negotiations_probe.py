...
#: Потолок страниц резолв-обхода topic→chat (#1154): поиска ОДНОГО топика по
#: списку (probe --negotiations --topic, robot-reply). Конец списка доказывается
#: данными — ноль новых тем + overlap-страж ``_require_windows_overlap`` — а не
#: этим числом; потолок только defense-in-depth от патологического сервера,
#: который никогда не отдаёт пустую/повторную страницу. Живой факт 2026-09-19:
#: список «Все» = 485 тем = 49 страниц, а дефолтные капы 5/20 резали глубину —
#: тихие с 2026-09-08 чаты (сортировка по активности) выглядели «исчезнувшими»,
#: на самом деле сидя на страницах 44–48. Требуемая глубина растёт вместе с
#: аккаунтом (~700 откликов за 2.5 недели): 200 страниц × 10 тем = 2000 тем.
#: При превышении резолв-путь снова начнёт [FAIL]-ить «не найден» — это видно,
#: потолок поднимается одной константой.
RESOLVE_TOPIC_MAX_PAGES = 200
