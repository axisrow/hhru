...
class History:
    ...
    def list_questionnaire_audit(
        ...
        *,
        ...
        при этом НЕ восстановима: так пишется и ответ ниже порога, и отказ
        комплаенс-гейта — ``answerer._queue`` отдаёт любому нерешённому вопросу
        ``AnswerProposal("", 0.0)``. Отсюда и формулировка флага «не стал
        отвечать», без указания причины.
