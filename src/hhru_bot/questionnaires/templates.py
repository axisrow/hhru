...
_COMPLIANCE_PATTERN = re.compile(
    ...
    r"достоверн|"
    # Упоминание РФ/России в вопросе к кандидату. Признак намеренно широкий:
    # это НЕ перечень предикатов. Ревью PR #487 показало, что список глаголов
    # тут не сходится — «на территории РФ», «проживаете в РФ», «живёте в РФ»,
    # «находитесь в РФ» спрашивают один и тот же правовой статус пребывания, и
    # каждый следующий раунд приносил новую словоформу, отвеченную городом из
    # обычного ``location``. Признак по самой стране закрывает класс целиком.
    #
    # Ширина безопасна ровно по документированной здесь асимметрии: ложное
    # срабатывание уводит вопрос в очередь к человеку, пропуск — отправляет
    # работодателю уверенную догадку. На корпусе из 146 собранных вопросов
    # сверх резидентских добавляется один — «оформлены по ТК РФ?», и он тоже
    # про правовой статус, то есть попадает сюда по делу, а не случайно.
    r"\bрф\b|росси",
    re.IGNORECASE,
)
...
SEED_TEMPLATES: tuple[SeedTemplate, ...] = (
    _seed(
        ...
        # Одного \b мало: дефис и слэш сами являются границей слова, поэтому
        # «ЗП-модуль»/«ЗП/кадры» проходили бы как зарплатный вопрос, хотя это
        # название предметной области. Соседство с ними исключено явно.
        r"(?<![\w/-])зп(?![\w/-])",
        r"(?<![\w-])з/п(?![\w-])",
    ...
    _seed(
        ...
        # Формы «проживаете в РФ / в России» сюда НЕ входят: это вопрос о
        # правовом статусе пребывания, он удерживается комплаенс-гейтом
        # (``_COMPLIANCE_PATTERN``) наравне с «на территории РФ».
