...
SEED_TEMPLATES: tuple[SeedTemplate, ...] = (
    _seed(
        ...
        r"заработн\w*\s+плат",
        r"оклад",
        r"уровень\s+(дохода|доход\w*|оплат\w*)",
        r"ожидани\w*\s+по\s+доход",
        r"желаем\w*\s+доход",
        r"сколько\s+\w*\s*(хотите|хотели|рассчитываете)\s+(зарабатывать|получать)",
    ...
    _seed(
        ...
        r"город\w*\s+прожив",
        r"стран\w*\s+прожив",
        r"где\s+вы\s+(живёте|живете|проживаете|находитесь)",
        r"ваш\s+город",
        r"город\s+и\s+стран",
    ),
    _seed(
        "desired_role",
        "motivation",
        r"желаем\w*\s+(должност|рол)",
        r"как\w*\s+рол\w*\s+\w*\s*интерес",
        r"рол\w*\s+\w*\s*интересу",
        r"какие\s+задачи",
        r"чем\s+хотите\s+заниматься",
        r"интересующ\w*\s+позици",
    ),
    _seed(
        "business_segments",
        "marketing",
        r"сегмент\w*\s+бизнеса",
        r"в\s+каких\s+сферах",
        r"с\s+какими\s+нишами",
        r"опыт\s+в\s+сегмент",
        r"\bb2b\b.*\bb2c\b",
        r"\bb2c\b.*\bb2b\b",
    ),
)
