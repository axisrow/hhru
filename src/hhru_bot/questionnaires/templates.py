...
_COMPLIANCE_PATTERN = re.compile(
    ...
    # Слово «территория» в формулировке необязательно: «проживаете в РФ?»
    # спрашивает ровно тот же статус. Пока эта форма относилась к обычному
    # ``location``, соседние формулировки одного смысла расходились на два
    # исхода — одна уходила в очередь, другая отвечалась сохранённым городом с
    # keyword-уверенностью выше порога, то есть ответом не на заданный вопрос.
    r"территори\w*\s+рф|на\s+территории\s+(рф|росси)|"
    r"прожива\w*\s+в\s+(рф\b|росси)",
    re.IGNORECASE,
)
...
SEED_TEMPLATES: tuple[SeedTemplate, ...] = (
    ...
    _seed(
        ...
        # живёте», но и «откуда планируете работать».
