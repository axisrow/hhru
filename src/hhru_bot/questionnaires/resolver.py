...
#: Источники сопоставления, которые НЕ являются решением человека. На
#: комплаенс-теме такой матч не даёт права ответить сохранённым значением: за
#: ним не стоит ни явно объявленный строгий кластер, ни подтверждённая
#: пользователем формулировка. ``PHRASE`` сюда не входит намеренно — это
#: однажды подтверждённое человеком соответствие.
_GUESSED_SOURCES: frozenset[str] = frozenset({KEYWORD, LLM})
...
def compliance_gate(
    ...
    *,
    ...
    # #482: «Ни ключевые слова, ни LLM, ни будущий ML-слот» не отвечают на
    # комплаенс. Проверки «шаблон static и непуст» для этого мало: составной
    # вопрос («проживаете в РФ? в каком городе?») распознаётся как комплаенс ПО
    # ТЕКСТУ, но keyword-эвристика уводит его в обычный ``location``, и
    # сохранённый там город удовлетворял гейт как «явное значение» — хотя
    # оператор заводил его для вопроса о городе, а не о правовом статусе.
    # Значение считается явным, только если сам шаблон объявлен строгим либо
    # выбран решением человека (подтверждённая формулировка), а не догадкой.
    if match is not None and not is_strict(match.cluster) and match.source in _GUESSED_SOURCES:
        return f"комплаенс-вопрос сопоставлен догадкой, а не явным шаблоном{name}"
    if template is None:
