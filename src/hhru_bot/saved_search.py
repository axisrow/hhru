...
Кнопка «Сохранить поиск» на /search/vacancy открывает попап (поле имени,
способ уведомлений, финальный submit). Живым census 2026-09-08 подтверждена
только сама кнопка и страница списка автопоисков /applicant/autosearch;
попап рендерится в DOM исключительно после клика и его селекторы НЕ
подтверждены. Поэтому боевой путь честно останавливается ДО клика — тот же
fail-closed контракт, что у ``report_vacancy`` (#745): неподтверждённый
мутационный клик не выполняется. Как только селекторы попапа будут сняты
живым DOM, ``save_search_on_hh`` дополнится шагами попапа.
...
def save_search_on_hh(
    ...
    *,
    ...
    Боевой путь: НЕ КЛИКАЕТ. Селекторы попапа сохранения (поле имени, submit)
    не подтверждены живым DOM, и клик по кнопке открытия — единственная
    неподтверждённая граница этого флоу: за ней локальные таймауты ничего
    не доказывают (#207), а подтвердить результат нечем. Fail-closed отказ
    ``acted=False`` — то же правило, что у report_vacancy (#745). ``before_click``
    ...
    return SaveSearchResult(
        ...
        acted=False,
        reason=(
            "селекторы попапа сохранения не подтверждены живым DOM (#1052): "
            "боевой клик «Сохранить поиск» не выполняется. Снимите census попапа "
            "после разведочного клика и дополните save_search_on_hh"
        ),
    )
