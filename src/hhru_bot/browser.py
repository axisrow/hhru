...
def require_authenticated_page(
    ...
    *,
    ...
    this check succeeds.
    """
    # #1194: агентский воркер без кук в своём пустом профиле циклит «login»,
    # хотя интерактивный вход ему недоступен. Подсказка в самом тексте отказа
    # направляет к реальному рецепту — сессия живёт в storage_state-файле.
    # Семантика гейта (#147/#1129) не меняется, только текст; ветка с
    ...
    if not (auth_cookie_check or has_auth_cookie)(page):
        raise NotAuthenticated(
            "cookie hhtoken не найден — сессия истекла (запустите `login`, затем "
            "повторите). Агентский воркер: интерактивный login недоступен — "
            "сессия hh.ru живёт в файле storage_state/hh_session.json аккаунта: "
            "запускайте CLI с конфигом аккаунта (файл подхватится сам) либо "
            "перенесите куки в открытый профиль посевом (add_cookies), затем "
            "повторите"
        )
    if (login_form_check or has_login_form)(page):
        raise NotAuthenticated(
            "страница содержит форму входа при наличии hhtoken — сессия отвергнута "
            "сервером (запустите `login`, затем повторите). Агентский воркер: "
            "hhtoken есть в jar, но сервер его отверг — пересев того же "
            "storage_state не поможет; нужен свежий `login` человеком или свежая "
            "сессия через `import-cookies` из залогиненного Chrome"
        )
...
def wait_for_named_control_hydration(
    ...
    #1030: единственный потребитель (тоггл «Фильтров») переехал на data-qa и
    wait_for_react_hydration; хелпер оставлен осознанно — это штатный путь
    гидрации для будущего контрола, у которого data-qa нет вовсе.
