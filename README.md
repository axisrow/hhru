...
- `--text` — Готовый текст «Обо мне» без LLM (#326): ai_profile и секция ai не требуются
...
- `--institution`/`--faculty`/`--specialty`/`--year` — Готовая запись основного образования без LLM (#326)
- `--primary-entry`/`--additional-entry` — Готовые записи образования JSON без LLM (можно повторять, #326)
...
- `--entry` — Готовая запись опыта JSON без LLM (#326), можно повторять; требует непустые company и position
...
- `--resume` — Slug из конфига или реальный resume_id HH.ru (#319)
- `--mode` —  (по умолчанию: 'fill')
- `--title`/`--salary`/`--currency`/`--employment`/`--work-format`/`--commute`/`--business-trips`/`--specialization` — Готовые значения раздела без LLM (#326); любое из них отключает LLM-планирование
...
- `--resume` — Slug из конфига или реальный resume_id HH.ru (#319)
- `--attestation`/`--recommendation` — Готовые записи JSON без LLM (#326), можно повторять
