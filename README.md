...
2. **Запусти сервер канала.** `./scripts/run.sh live-serve` — он живёт, пока
   открыта команда (Ctrl+C гасит). По умолчанию слушает `127.0.0.1:8765` —
   адрес, к которому расширение подключается жёстко; `--port 0` даёт
   ephemeral-порт, но расширение о нём узнать не может.
...
### opencode: команда и скиллы из коробки
...
- **Команда `/hhru`** (`.opencode/commands/hhru.md`) — тот же слэш-интерфейс,
  что у Claude Code / Codex plugin (примеры — в «Команда `/hhru`» выше).
...
- `--dry-run` — Показать, что будет сделано, без реальных действий
- `--max-pages MAX_PAGES` — Максимум страниц поиска (по умолчанию: 5)
- `--port PORT` — Порт канала на 127.0.0.1 (расширение слушает 8765) (по умолчанию: 8765)
...
Читает JSON-файл команды export-resume и в ТЕКУЩЕМ аккаунте (--account) создаёт резюме: каркас через визард создания, роль — по профессии из экспорта v2 (position.role; в экспортах без неё — по title), затем секции — позиция (editor-режим), опыт, образование, навыки, языки, о себе, фото, а также блоки из экспорта v2: контакты, сертификаты и портфолио (per-row исходы; фото портфолио загружаются в галерею аккаунта, ссылки-строки портфолио не поддержаны). Секция, которую невозможно перенести без выдумывания значения, честно пропускается. Экспорт схемы v1 импортируется как раньше — блоковые секции не переносятся. Финальный отчёт сверяет экспорт и импорт по секциям. WRITE-команда: по умолчанию только dry-run; боевой запуск требует --force. Никаких удалений.

- `--file FILE` — Путь к JSON-файлу экспорта (schema export-resume/v1 или /v2)
...
- `--wait-seconds WAIT_SECONDS` — Сколько секунд ждать подключения расширения (по умолчанию: 10.0)

### `live-serve`

- `--port PORT` — Порт на 127.0.0.1 (по умолчанию 8765 — согласован с расширением) (по умолчанию: 8765)
...
- `--max-pages MAX_PAGES` — Максимум страниц списка откликов (по умолчанию — адаптивный: конец списка доказывается данными, backstop 50; у потолка при непустой странице — отказ, а не тихая обрезка, #1074)
...
Заполняет аттестации, рекомендации, сертификаты и портфолио (#1121) по подтвержденным UI-фактам; умеет создать первую строку в пустом блоке. Ссылки пока пропускаются, удаления не выполняются. Ключ manual в секции resume_sections конфига пока зарезервирован и ни на что не влияет — расцветёт вместе с блоковыми ишью #1119-1122.
...
- `--contact` — Контакт JSON (#1119, census-схема), можно несколько: '{"type":"phone|email", "value":..., "comment":... (только phone), "preferred":true|false}'. Замещает значение поля формы; preferred=True максимум у одной строки и переставляет радио предпочтительного способа на этот контакт (снимает с другого); без preferred существующий выбор hh.ru не меняется (#1124). Российский номер не принимается: hh.ru требует SMS-подтверждение. Селекторы подтверждены census 2026-09-12.
- `--certificate` — Готовый сертификат JSON (#1120), можно несколько: '{"name":..., "year":..., "url":...}'. Схема по read-only census 2026-09-12: у формы три поля, url опционален (файл сертификатом загрузить нельзя — input[type=file] в DOM нет, ссылка единственный путь подтверждения).
- `--portfolio` — Единица портфолио JSON (#1121), можно несколько: {"type":"image","photo_id":"..."} или {"type":"link","title":"...","url":"..."}. Схема по census 2026-09-12/14: ролей/периодов/описания в UI hh.ru нет; изображение — только выбор уже загруженного в галерею (/applicant/gallery) фото; UI для ссылок не существует — link-строки дают [FAIL] fail-fast. Image-строка: выбор работы в модалке + Сохранить + readback.
...
- `--unreachable` — Снять строку очереди БЕЗ вердикта: чат недостижим (топик исчез из negotiations — вакансия недоступна/переписка удалена, #1154). robot_verdicts не пишется
...
- `--max-pages MAX_PAGES` — Нижняя граница страниц negotiations для SSR topic→chat mapping: резолв-обход ищет топик до доказуемого конца списка, малое значение глубину не режет (потолок 200) (по умолчанию: 5)
...
- `--url` — Страница, где видна капча (по умолчанию https://hh.ru) (по умолчанию: 'https://hh.ru')
- `--wait-seconds WAIT_SECONDS` — Сколько секунд держать окно открытым (по умолчанию 300) (по умолчанию: 300)
...
Передаёт файл в скрытый file-input кнопки «Добавить работу» (/applicant/gallery). WRITE-hh-ru: по умолчанию только dry-run (осмотр мишени загрузки); боевой запуск требует --force или интерактивного подтверждения. Успех — только появление новой карточки gallery-image-{id}; её id печатается для --portfolio.

- `--photo PHOTO` — Путь к файлу jpg/jpeg/png
- `--dry-run` — Осмотреть мишень загрузки, ничего не загружая
- `--force` — Подтвердить боевую загрузку
