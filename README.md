...
Активный файл ротируется при достижении 10 MiB: прежний сегмент переименовывается
в `hhru_bot.log.1`, `.2` и т. д. Архивы не удаляются автоматически — даже после
достижения настроенного числа бэкапов они сохраняются, а ручная очистка остаётся
