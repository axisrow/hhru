...
CLI-инструмент для поиска работы на hh.ru: ищет вакансии по твоим фильтрам,
откликается с персонализированным письмом, поднимает резюме в поиске, следит
за ответами работодателей и умеет редактировать само резюме (опыт, образование,
навыки, должность, публикацию) через тот же браузерный UI, которым пользуется
человек. Работает через Playwright, а не через закрытый с декабря 2025
соискательский API hh.ru — то же ограничение, с которым живут все актуальные
инструменты этого класса в 2026 году.

Управляется только из терминала — никакого веб-дашборда или фонового демона.
Для регулярного запуска (например, поднимать резюме каждые 4 часа) подключается
внешний планировщик — cron/launchd или Docker; сам инструмент не хранит скрытого
«активного» состояния между запусками. Это осознанная защита, а не недостаток:
каждая команда печатает, что именно она делает, поддерживает `--dry-run` для
проверки плана без единого клика по hh.ru, и ограничена дневными лимитами и
случайными паузами между действиями — чтобы обычная работа с сайтом не выглядела
для анти-фрод системы hh.ru как подозрительная автоматизация. Опасные действия
(отклик, публикация, удаление резюме) без явного `--force` или интерактивного
подтверждения не выполняются никогда.

Полный список из 57 команд — в «Справочник команд» ниже; чем это отличается от
дюжины похожих ботов — в разделах «Сравнение с референс-проектами» и «Чего нет
у референсов» дальше по README.

## Сравнение с референс-проектами

Полный список изученных проектов и ссылки на них — в разделе «Похожие проекты
(для идей)» ниже; здесь — сводная таблица по возможностям. Столбцы ограничены
теми пунктами, которые реально зафиксированы в README/`docs/cli-spec.md`/
`docs/research/` по каждому проекту: если про проект нет подтверждённой записи —
в ячейке стоит `н/д` (не проверяли), а не «нет». Утверждение «нет» ставится
только там, где отсутствие явно зафиксировано (например, «после декабря 2025 не
работает» — это про API-путь, а не про весь проект).

| Проект | Стек | Поиск + отклик | Поднятие резюме | Троттлинг/лимиты | Редактирование резюме | Анкеты работодателя | Анализ резюме конкурентов |
|---|---|---|---|---|---|---|---|
| **hhru-bot (этот проект)** | Python + sync Playwright | да | да, кулдаун 4ч | дневные лимиты + случайные паузы | да: опыт, образование, навыки, языки, должность, публикация, создание/удаление/переименование | да, обучаемые шаблоны + LLM-фолбэк | да (`competitors`) |
| [s3rgeym/hh-applicant-tool](https://github.com/s3rgeym/hh-applicant-tool) | Python, гибрид API + Playwright | да | да | да (эталон схемы SQLite и троттлинга) | только клонирование целиком (`clone-resume`) | н/д | н/д |
| [fikstt2/hh-ai-agent](https://github.com/fikstt2/hh-ai-agent) | Python + Playwright | да | н/д | н/д | н/д | н/д | н/д |
| [YAMAKAYAMACO/hh-autoresponder](https://github.com/YAMAKAYAMACO/hh-autoresponder) | Python + Playwright + SQLite | да, rule-based скоринг без LLM | н/д | да | н/д | н/д | н/д |
| [Steev193/hh-ru-apply](https://github.com/Steev193/hh-ru-apply) | Node + Playwright | да | н/д | н/д | н/д | н/д | н/д |
| [tgeruzov/hh-auto-responder](https://github.com/tgeruzov/hh-auto-responder) | Tampermonkey userscript | да, с роутингом трёх исходов отклика | н/д | н/д | н/д | н/д | н/д |
| [semernyakov/hh-auto-apply](https://github.com/semernyakov/hh-auto-apply) | Python + sync Playwright | да | да, с парсером cooldown из текста hh.ru | н/д | н/д | н/д | н/д |
| [beatwad/XX_Auto_Jobs_Applier](https://github.com/beatwad/XX_Auto_Jobs_Applier) | Python + Playwright | да, config-driven (YAML) | н/д | н/д | н/д | н/д, есть решение капчи через Telegram | н/д |
| [Vlad9572324/hh.ru-clicker](https://github.com/Vlad9572324/hh.ru-clicker) | н/д (API-путь) | н/д | да, через API `/applicant/resumes/touch` (нужна проверка актуальности после дек. 2025) | сброс лимита в 00:00 МСК | н/д | н/д | н/д |
| [rycln/hhraiser](https://github.com/rycln/hhraiser) | Go | н/д | да | джиттер расписания против антифрода | н/д | н/д | н/д |

`н/д` в этой таблице означает буквально «мы не проверяли эту возможность у
проекта», а не «у него точно нет функции». Ни один из перечисленных проектов
мы не аудировали построчно — таблица основана на их README и на нашем
разборе кода по конкретным вопросам (см. `docs/research/`), а не на полном code
review. У `s3rgeym/hh-applicant-tool` больше 500 звёзд и он прямо назван в этом
README эталоном по шаблонам сопроводительных писем, схеме SQLite и троттлингу —
широта нашего набора команд не означает, что решения хуже.

## Чего нет у референсов

Пункты ниже проверены по фактическому составу `src/hhru_bot/commands/` (57
модулей) — если функция не описана здесь одной-двумя строками, спецификация в
`docs/cli-spec.md` или справочник команд ниже дают дополнительные детали.

- **Редактирование и создание резюме прямо в hh.ru.** `create-resume`,
  `edit-experience`, `edit-education`, `edit-skills`, `edit-languages`,
  `resume-position`, `resume-sections`, `resume-visibility`, `publish-resume`,
  `rename-resume` — можно собрать резюме с нуля или довести существующее до
  публикации, не открывая браузер руками. У референсов из resume-функций
  встречаются только просмотр списка резюме и клонирование резюме целиком
  (`s3rgeym clone-resume` — аналог нашего `copy-resume`). Причина разрыва
  зафиксирована в этом README ниже: 15 декабря 2025 hh.ru закрыл соискательский
  API, включая работу с резюме, а браузерным путём в эту сторону, судя по
  изученным проектам, никто не пошёл.
- **Анализ рынка резюме конкурентов** (`competitors collect`/`competitors
  report`) — видно не только вакансии, но и с кем ты конкурируешь за них:
  собирает обезличенные профессиональные поля резюме других соискателей по
  запросу и строит локальный отчёт по ролям, зарплатам и навыкам. Подробности —
  в блоке ниже. Ни у одного изученного референса такой функции нет.
- **Обучаемые ответы на анкеты работодателей** (`questionnaire`, пакет
  `questionnaires/`) — если работодатель просит заполнить анкету при отклике,
  бот сам подбирает ответ по ранее подтверждённым формулировкам, а для новых
  вопросов подключает LLM-подсказку; неуверенные случаи уходят в очередь на
  твоё решение, а не отправляются наугад.
- **Честный отчёт о неопределённом результате** (`uncertain`, `diagnostics`).
  Если во время клика по кнопке отклика/удаления/публикации браузер потерял
  связь с hh.ru и не может подтвердить итог, бот не пишет тебе «готово» или
  «не готово» наугад — он прямо говорит «не знаю, дошло ли действие», и позже
  через `uncertain reconcile` эти случаи можно свести к достоверному статусу.
  Это дороже в реализации, но не даёт тихо продублировать отклик или решить,
  что резюме удалилось, когда оно осталось на месте.
- **Воронка и статистика по всей истории** (`funnel`, `stats`, `responses`,
  `resume-views`, `market`) — не разовый прогон, а накопленная картина: сколько
  откликов ушло, сколько ответов пришло, где «мёртвая зона» без ответа, как
  вакансии распределены по зарплатам.
...
Имена, контакты, фото, демография, местоположение, активность, полный текст
страницы и HTML не сохраняются; hh.ru используется только на чтение, результат
остаётся локальным.
...
Инструмент поддерживает несколько независимых профилей hh.ru на одной
машине — например, свой аккаунт и аккаунт другого соискателя. Аккаунт
задаётся именем и живёт под `data/accounts/<name>/`.

### Второй (и последующий) аккаунт

```bash
hhru account create marketing
```

Команда создаёт `data/accounts/marketing/config.yaml` из шаблона и не
перезаписывает существующий аккаунт. Отредактируй новый конфиг (резюме,
фильтры поиска, дневные лимиты — как обычный `data/config.yaml`), затем
войди под этим именем:
...
`--account`, а не в текущий/последний использованный.

### Как выбрать аккаунт

Глобальный флаг `--account <name>` ставится **до** имени подкоманды:

```bash
./scripts/run.sh --account marketing apply --resume resume-name-1 --limit 5
```

Для плановых задач (cron/launchd/Docker) то же самое делает переменная
окружения `HHRU_ACCOUNT` — её понимает `scripts/scheduled_run.sh`:

```bash
HHRU_ACCOUNT=marketing scripts/scheduled_run.sh --headless apply --limit 5
```

Если заданы оба варианта, явный `--account` в аргументах имеет приоритет
над `HHRU_ACCOUNT`.

Отдельной команды «переключить текущий аккаунт» (`account switch`) в
проекте нет осознанно: скрытое состояние «активный аккаунт» сделало бы
плановые задачи невоспроизводимыми — какой аккаунт отработает в 3 часа
ночи, зависело бы от того, что кто-то вручную переключил днём. Аккаунт
всегда указывается явно, вызовом за вызовом.

Список локально настроенных аккаунтов и их состояние (наличие сессии,
последнее действие) показывает `account list`:

```bash
./scripts/run.sh account list
```

### Изоляция

У каждого аккаунта — полностью свой набор состояния:
- своя сессия hh.ru (`data/accounts/<name>/config.yaml` →
  `storage_state_file`, по умолчанию `data/accounts/<name>/storage_state/hh_session.json`);
- своя история откликов и поднятий (`data/accounts/<name>/history.db`);
- свои дневные лимиты откликов/поднятий (`throttle.daily_apply_limit`,
  `throttle.daily_bump_limit` в конфиге аккаунта).

Лимиты считаются по истории именно того аккаунта, под которым выполняется
команда — они **не суммируются и не переносятся** между аккаунтами: 40
успешных откликов на аккаунте `default` не расходуют дневной лимит
аккаунта `marketing`, и наоборот.

### Параллельный запуск

Команды, изменяющие состояние (`apply`, `bump`, `run` и другие WRITE-команды),
сериализуются файловой блокировкой, скоупленной по директории `history.db`
запущенной команды. Поскольку у каждого именованного аккаунта своя
`history.db`, из этого следует:
- **разные аккаунты можно гонять параллельно** — `--account default` и
  `--account marketing` не блокируют друг друга;
- **два одновременных прогона ОДНОГО аккаунта запрещены** — второй получит
  `[FAIL] другой процесс уже выполняет WRITE-действие`.

### Секреты

Файл сессии аккаунта (`hh_session.json`) даёт доступ к hh.ru наравне с
паролем. Вся папка `data/`, включая `data/accounts/`, целиком в
`.gitignore` — новый аккаунт не требует правки `.gitignore` и не может
случайно уехать в коммит. Файлы сессии создаются с правами `0600`, каталог
аккаунта — с правами `0700` (см. `diagnostics doctor` для проверки прав уже
существующих файлов).

### Как убрать аккаунт

Отдельной команды удаления аккаунта нет. Чтобы полностью убрать локальный
профиль, удали каталог `data/accounts/<name>/` вручную — это разом сотрёт
его сессию, историю и конфиг.
...
Сквозной путь от планировщика до записи события в календаре — только READ
до последнего шага, WRITE ограничен явным подтверждением пользователя.

1. **Планировщик опрашивает hh.ru** (см. «Автопилот» выше) и распознаёт новое
   приглашение по коду выхода `CommandExitCode.NEW_INVITATIONS` (10):

   ```bash
   ./scripts/run.sh --headless responses --alert-new
   ```

   Первый запуск фиксирует baseline (ничего не сообщает про уже существующую
   историю), любой следующий сообщает только про *новые* приглашения с
   момента последнего успешного опроса. `scripts/scheduled_run.sh` уже умеет
   распознавать этот код: пишет `***** НОВОЕ ПРИГЛАШЕНИЕ *****` в
   `data/logs/scheduled.log` и опционально вызывает `HHRU_ALERT_CMD` (#707/#708).

2. **`responses` без `--alert-new`** — обычный просмотр «что нового» с ASCII-
   таблицей вакансия/работодатель/статус/дата:

   ```bash
   ./scripts/run.sh responses
   ```

3. **`learn`** — что подтянуть по требованиям из уже собранных вакансий
   (агрегат по всей истории `search`, а не привязка к одной вакансии из
   приглашения — привязать точечно нечем, hh.ru не связывает приглашение с
   текстом вакансии на уровне API):

   ```bash
   ./scripts/run.sh learn --resume <id>   # --resume исключает уже указанные в резюме навыки
   ```

4. **Calendar event вручную.** hh.ru не сообщает время собеседования в самом
   статусе приглашения, поэтому CLI никогда не угадывает его — `--calendar-hint`
   печатает готовую команду с плейсхолдерами времени, пользователь подставляет
   реальные `--start`/`--end` сам:
...
Отдельно, когда работодатель молчит после отклика/собеседования — не часть
цепочки выше (не завязано на приглашение), но тот же класс сценария:
`reply-employers --follow-up --after-days N` (#710) напоминает о себе только
там, где последнее слово уже за нами и hh.ru явно разрешает напоминание
(`responseReminderState.allowed`, тот же признак, что у `responses --remindable`):
...
Автоматизирована часть цепочки «пришло приглашение -> follow-up» — см. раздел
«Сценарий: пришло приглашение» выше для реальных команд. Само собеседование
(нарратив, поведенческие вопросы, портфолио) остаётся **вне бота** и делается
пользователем вручную — здесь бот не готовит к интервью.

Что уже реализовано (не бэклог):

- `responses --alert-new` — сигнал о новом приглашении для планировщика (#707);
- `learn` — что подтянуть: агрегат частых требований/скиллов из собранных
  вакансий, с исключением уже указанных в резюме навыков (#391);
- `responses --calendar-hint` — готовая команда `calendar event` для нового
  приглашения, время всегда плейсхолдер (#711);
- `reply-employers --follow-up --after-days N` — напоминание работодателю,
  когда последнее слово за нами и он молчит N дней (#710).

Ручной чек-лист пользователя (вне бота):

- подготовить проекты и портфолио под требования, которые показал `learn`;
- подготовить нарратив («расскажите о себе») и ответы на поведенческие вопросы;
- поставить реальное время интервью в `calendar event` вместо плейсхолдера.
...
Генерирует заголовок, «Обо мне», порядок навыков и отбор мест работы/проектов под один из четырёх кластеров вакансий (#752). Всегда показывает результат как предложение; запись на hh.ru эта команда не делает (см. #753 PR-1/PR-2).

- `--resume` — Slug из конфига или реальный resume_id HH.ru (#319)
- `--cluster` — Кластер вакансий, под который адаптируется резюме
- `--dry-run` — Явно показать план без сохранения (поведение по умолчанию — эта команда не пишет на hh.ru в принципе; флаг оставлен для единообразия с другими командами и совместимости с будущим PR-2)
