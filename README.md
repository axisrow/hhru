...
Изменяет видимость одного резюме на hh.ru. Опасная WRITE-команда: боевой режим
требует `--force` или интерактивного подтверждения; до подтверждения селектора
блока видимости запись недоступна (fail-closed).

- `--resume` — Slug из конфига или resume_id HH.ru
- `--mode` — `everyone`, `no-one`, `link-only`, `whitelist` или `blacklist`
- `--dry-run` — Показать план без записи
- `--force` — Подтвердить боевую запись
