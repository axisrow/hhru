...
### Автозаполнение анкет работодателей

Функция выключена по умолчанию. Включите keyword-resolver в `data/config.yaml`:
...
Встроенные правила распознают зарплатные ожидания, город, желаемую роль и
сегменты бизнеса. Подтверждённые ответы хранятся в `history.db`: общий ответ
действует на аккаунт, а `--resume` создаёт переопределение для резюме. Для LLM
fallback дополнительно включите `ai.answer_questions: true`. Первое LLM-
сопоставление всегда требует подтверждения; паспортные, гражданские, воинские и
другие compliance-ответы LLM не генерирует.

```bash
# Посмотреть встроенные/изученные шаблоны и очередь неизвестных вопросов
./scripts/run.sh questionnaire templates --resume python
./scripts/run.sh questionnaire pending --resume python

# Разобрать сохранённую очередь интерактивно
./scripts/run.sh questionnaire learn --resume python

# Задать общий ответ или override конкретного резюме
./scripts/run.sh questionnaire set location --mode static --answer "Москва"
./scripts/run.sh questionnaire set salary --resume python --mode contextual \
  --instruction "Укажи диапазон из настроек кандидата, без выдуманных фактов"

# Обучаться прямо во время безопасного предпросмотра формы
./scripts/run.sh apply --resume python --dry-run --learn-questionnaires --limit 5
```

Без TTY или в `--headless` неизвестный вопрос записывается в `questionnaire
pending`, вакансия пропускается, а обработка следующих продолжается. Реальная
отправка формы с автоматически заполненной анкетой по-прежнему требует
отдельного `--force`; сначала обязательно проверьте тот же запуск с `--dry-run`.
...
- `--force` — Разрешить реальную отправку отклика с автоматически заполненной анкетой
- `--limit LIMIT` — Целевое число успешных откликов за запуск (0 = без ограничения кроме дневного лимита)
- `--learn-questionnaires` — Интерактивно подтверждать новые шаблоны вопросов (только TTY, не headless)
