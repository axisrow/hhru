...
- `--specialization` — Специализация (можно несколько); селектор hh.ru не подтверждён — запись упадёт fail-closed
- `--salary SALARY` — Зарплата (целое число, без LLM)
- `--currency` — Валюта зарплаты
- `--employment` — Тип занятости (можно несколько)
- `--work-format` — Формат работы (можно несколько)
