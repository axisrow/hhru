...
Нажимает кнопку быстрых ответов под вопросом робота в чате (очередь: robot-queue). WRITE-hh-ru: боевой клик требует --force или подтверждения; dry-run показывает план без клика.

- `--topic` — ID topic из robot-queue
- `--answer` — Точный текст кнопки (например «Да» или «Нет»)
