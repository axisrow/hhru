...
Ручное решение капчи hh.ru: открывает ЗАЛОГИНЕННОЕ headful-окно браузера на странице с капчей, держит его открытым заданное время — капчу решает только человек (кликов за бота нет). По истечении окна сессия сохраняется тем же безопасным путём, что и `refresh-token --force`. WRITE-local. Когда анти-бот hh.ru отдаёт страницы вакансий через `/captcha` и боевой прогон остановлен отказом «решите её вручную и повторите запуск» — это и есть штатный путь «вручную» (#1146).

- `--url URL` — Страница, где видна капча (по умолчанию https://hh.ru)
- `--wait-seconds N` — Сколько секунд держать окно открытым (по умолчанию 300)
