...
Открывает визард hh.ru и создаёт новый черновик резюме. WRITE-команда: по умолчанию только dry-run; боевой запуск требует --force или интерактивного подтверждения.

- `--area` — Профобласть первого шага визарда
- `--title` — Желаемая должность первого шага
